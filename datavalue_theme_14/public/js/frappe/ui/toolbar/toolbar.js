...
frappe.ui.toolbar.Toolbar = class {
    ...
    make() {
        ...
        $('.dv-navbar .dv-nav-right .chat-navbar-icon').hide();
        setTimeout(() => {
            console.log("Chat status", ('desk' in frappe) && ('Chat' in frappe))
            if (('desk' in frappe) && ('Chat' in frappe)) {
                const navbar_icon_html = `<span style="position:relative;top:10px;padding: 0px 5px;">${frappe.utils.icon('small-message', 'md')} <span class="badge" id="chat-notification-count"></span></span> `;
                $('.dv-navbar .dv-nav-right .chat-navbar-icon').prepend(
                    navbar_icon_html
                );
                $('.dv-navbar .dv-nav-right .chat-navbar-icon').show();
            } else {
                $('.dv-navbar .dv-nav-right .chat-navbar-icon').hide();
            }
        }, 1000);
